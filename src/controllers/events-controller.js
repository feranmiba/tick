...
export const attendEvent = async (req, res) => {
    ...
    try {
      ...
      if (result.rows.length > 0) {
        const { user_id, event_id, qrcode_url, token: eventToken, email: userEmail } = result.rows[0];
  
        const userNameResult = await db.query("SELECT name FROM userprofiles WHERE user_id = $1", [user_id]);
        const eventResult = await db.query("SELECT event_name, picture, date, time_in, event_address, price, type FROM eventcreation WHERE id = $1", [event_id]);
  
        if (userNameResult.rows.length && eventResult.rows.length) {
          const userName = userNameResult.rows[0].name;
          const { event_name, picture, date, type, price } = eventResult.rows[0];
  
          
          const htmlContent = loadEmailTemplate("ticket", {
            userName,
            ...
            eventDate: date,
            eventToken,
            eventImageUrl: `https://app.swiftjobs.com.ng/${picture}`,
          });
  
         
          await sendEmail(userEmail, htmlContent, `${event_name} Ticket Confirmation`);
  
          return res.status(200).json({ message: "Ticket email sent successfully" });
        } else {
